...
\begin{definition}
  \label{1D_matrix}
  A 1D matrix, {\tt M}, is defined using a type {\tt T} and positive integer
...
\begin{definition}
  \label{2D_matrix}
  A 2D matrix, {\tt M},  is defined using a type {\tt T} and positive integers 
\(n_X, n_Y\), representing the dimensions/shape.
\begin{itemize}
  \item \(\mathtt{M}[x]\) is a vector of length \(n_Y\) and type {\tt T}
