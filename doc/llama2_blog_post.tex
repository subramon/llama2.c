...
As is often the case with performance tuning, intuition is useful but
experimentation is vital. 
We break up the changes we made into two sections ---
those that didn't matter and those that did.

\subsection{Changes that didn't matter}

These changes seemed logical and promising but didn't yield any 
significant performance gain. 
we list them because them here because we suspect that
their ``failure'' was largely because the code was simple enough that 
...
The hardware used for benchmarking, inspired by DHH's blog post
\footnote{ \url{https://world.hey.com/dhh/it-s-a-beelink-baby-243fdaf1}} 
was a 
...
\subsection{Comparing \ISPC\ versus gcc}
Benchmarking individual function shows that, for the most part, the contest
between \ISPC\ and gcc is a wash.
In Table~\ref{ispc_vs_gcc}, time taken by \ISPC\ is normalized to 1.
It is possible to use \ISPC\ for some functions and gcc for others.
\begin{table}
  \begin{tabular}{|l|l|l|} \hline \hline \\
    {\bf Function} & \ISPC\ & gcc \\ \hline \hline
    {\tt dot product} & 1.0 & 1.0 \\ \hline 
    {\tt softmax} & 1.0 & 1.40 \\ \hline 
    {\tt rmsnorm} & 1.0 & 0.70 \\ \hline 
    {\tt swiglu} & 1.0 & 1.25 \\ \hline 
\verb:x[i] += y[i] * s : & 1.0 & 1.03 \\ \hline
\verb:x[i] += y[i] : & 1.0 & 1.02 \\ \hline
\verb:y[i] = x[i] / s : & 1.0 & 0.6 \\ \hline
    \hline
    \end{tabular}
      \caption{Compaarison of time taken using \ISPC\ and gcc}
      \label{ispc_vs_gcc}
\end{table}
